function addOccupancyBarToDom(
  ...
  text.innerText = `B:${block} R:${room} Hay actualmente ${actual_occupancy} personas, de las ${Math.ceil(
    threshold * full_occupancy
  )} permitidas.
...
function sortRoomsByOccupancy(a, b) {
  actual_occupancy_a =
    a.payload === undefined ? 0 : JSON.parse(a.payload).actual_occupancy;
  ratio_occupancy_a = ratioOccupancy(
    actual_occupancy_a,
    a.full_occupancy,
    a.threshold
  );
  ...
  actual_occupancy_b =
    b.payload === undefined ? 0 : JSON.parse(b.payload).actual_occupancy;
  ratio_occupancy_b = ratioOccupancy(
    actual_occupancy_b,
    b.full_occupancy,
    b.threshold
  );
